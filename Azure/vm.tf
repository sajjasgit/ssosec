resource "azurerm_linux_virtual_machine" "ssosec_vm" {
  ...
  user_data             = base64encode(templatefile("${path.module}/scripts/install_app.sh", { APP_ACR_REPO_URL = azurerm_container_registry.ssosec_acr.repository_url, REGION = var.region }))
